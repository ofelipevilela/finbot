...
async function testSupabaseConnection() {
  try {
    ...
    logger.error("Erro ao conectar com Supabase", { err });
    return false;
  }
}
...
if (typeof useMultiFileAuthState !== "function") {
  throw new Error(
    "useMultiFileAuthState não pôde ser carregado. Verifique sua versão do Baileys."
  );
}
...
async function processFinancialMessage(
  ...
  if (BOT_CONFIG.COMMANDS.EXPENSE.some((cmd) => message.includes(cmd))) {
    ...
    if (valueMatch) {
      ...
      if (error) {
        logger.error("Erro ao inserir despesa", { error });
  ...
  if (BOT_CONFIG.COMMANDS.INCOME.some((cmd) => message.includes(cmd))) {
    ...
    if (valueMatch) {
      const valor = parseFloat(valueMatch[1].replace(",", "."));
      const descricao = descriptionMatch
      ...
      if (error) {
        logger.error("Erro ao inserir receita", { error });
  ...
  if (BOT_CONFIG.COMMANDS.BALANCE.some((cmd) => message.includes(cmd))) {
    ...
    return `💰 Seu saldo atual (${new Date().toLocaleDateString("pt-BR", {
      month: "long",
      year: "numeric",
    })}):\n\n📈 Receitas: R$ ${receitas.toFixed(
      2
  ...
  if (BOT_CONFIG.COMMANDS.REPORT.some((cmd) => message.includes(cmd))) {
    ...
    const despesasPorCategoria = despesas.reduce((acc: any, t) => {
      acc[t.categoria || "outros"] =
        (acc[t.categoria || "outros"] || 0) + (t.valor || 0);
      return acc;
    }, {});

    let relatorio = `📊 Relatório Financeiro - ${new Date().toLocaleDateString(
      "pt-BR",
      { month: "long", year: "numeric" }
    )}\n\n`;
    relatorio += `💰 Resumo:\n📈 Total de Receitas: R$ ${totalReceitas.toFixed(
      2
    ...
    if (Object.keys(despesasPorCategoria).length > 0) {
      ...
      Object.entries(despesasPorCategoria).forEach(
  ...
  if (BOT_CONFIG.COMMANDS.GOAL.some((cmd) => message.includes(cmd))) {
    ...
    if (valueMatch) {
      ...
      if (error) {
        logger.error("Erro ao definir meta", { error });
  ...
  if (categoriaMatch) {
    ...
    if (error) {
      logger.error("Erro ao buscar despesas por categoria", { error });
...
async function iniciarBot() {
  ...
  if (!supabaseConnected) {
    logger.error(
      "Não foi possível conectar com o Supabase. Verifique as credenciais."
    );
    return;
  }

  const { state, saveCreds } = await useMultiFileAuthState(
    BOT_CONFIG.AUTH_FOLDER
  );
  ...
  sock.ev.on("messages.upsert", async (m: any) => {
    logger.debug("Evento messages.upsert recebido", { m });
    ...
    try {
      ...
      const normalizePhone = (phone: string) => {
        ...
        if (normalized.length === 10) {
          normalized =
            normalized.substring(0, 2) + "9" + normalized.substring(2);
        }
      ...
      console.log("📊 Resultado da busca:", {
        userData: userData
          ? {
              id: userData.id,
              nome: userData.nome,
              telefone: userData.telefone_whatsapp,
            }
          : null,
        userError,
      });
      ...
      if (userError || !userData) {
        console.log(
          "❌ Usuário não encontrado para o telefone:",
          normalizedPhone
        );
  ...
  app.post("/api/enviar", async (req: Request, res: Response) => {
    ...
    if (!numero || !mensagem) {
      return res
        .status(400)
        .json({ erro: "Número e mensagem são obrigatórios." });
    }
  ...
  app
    .listen(BOT_CONFIG.PORT, () => {
      console.log(
        `🚀 Bot WhatsApp ativo em http://localhost:${BOT_CONFIG.PORT}`
      );
    ...
    .on("error", (err: any) => {
      if (err.code === "EADDRINUSE") {
        console.log(
          `⚠️ Porta ${BOT_CONFIG.PORT} em uso. Tentando porta ${
            BOT_CONFIG.PORT + 1
          }...`
        );
        app.listen(BOT_CONFIG.PORT + 1, () => {
          console.log(
            `🚀 Bot WhatsApp ativo em http://localhost:${BOT_CONFIG.PORT + 1}`
          );
          console.log("📱 Bot conectado e pronto para receber mensagens!");
        });
      } else {
